...
let Kiosk = (function(){
    ...
    let _createEvents = function(){
    	...
    	const $list = $('#kiosk-roll-video-list').sortable({
			update () {
				const newOrder = $list
					.children()
					.map((i, el) => $(el).data('id'))
					.get();
				Kiosk.rollVideos.sort((a, b) => newOrder.indexOf(a.id) - newOrder.indexOf(b.id));
			}
		});
        ...
        // Using keyboard to select hotkey
        $(window).keyup(event => {
			const which = event.which;
			const $openMenu = $('.hotKeySelect ul._show');
			if (which == 27) // esc key
				$openMenu.removeClass('_show');
			else
				$openMenu.children(`[data-hotkey=${which - 48}]`).click()
		});
    }
